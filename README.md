...
│   ├── routes/               # profile, generate, resume, pdf, template
...
3. **자동 생성 프로세스** (30~60초)
   - **1단계 (0-30%)**: AI가 프로필 분석하여 최적 템플릿 추천
   - **2단계 (30-60%)**: 선택된 템플릿으로 디자인 이력서 생성
   - **3단계 (60-90%)**: 컨설팅 리포트 작성
   - **4단계 (90-100%)**: 결과 저장 및 에디터 페이지로 이동
...
### 📋 이력서 히스토리 조회
1. "이력서 히스토리" 페이지 이동
2. 생성된 모든 컨설팅 리포트 목록 확인
3. 통계 정보 확인 (총 리포트 수, 유형별 개수)
4. 원하는 리포트 클릭 → 결과 페이지로 이동하여 확인
