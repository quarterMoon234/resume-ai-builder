# AI 기반 이력서 생성 & 컨설팅 웹서비스 (v0.8.0)

사용자 프로필을 입력하면 AI가 자동으로 **이력서**와 **컨설팅 리포트**를 동시에 생성하는 올인원 웹서비스입니다.
드래그 앤 드롭 에디터로 이력서를 자유롭게 편집하면서, AI 컨설턴트의 조언을 확인할 수 있습니다.

**최신 업데이트 (v0.8.0 - 2025-11-30)**:
- API 호출 최적화: 4번 → 2번으로 감소
- 데이터베이스 최적화: Resume 문서 생성 2개 → 1개로 감소
- 라우팅 개선: 이력서 히스토리에서 에디터로 직접 이동
- 코드 정리: 불필요한 페이지 및 필드 제거
...
- **API 최적화** (이력서 생성 API 호출 4번 → 2번으로 50% 감소)
