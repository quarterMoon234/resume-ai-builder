...
사용자가 프로필 정보를 입력하면 AI(OpenAI GPT)가 **이력서 초안**와 **컨설팅 리포트**를 동시에 생성해주는 올인원 웹 서비스입니다.

**핵심 특징:**
- **통합 생성**: 한 번의 클릭으로 이력서 초안 + 컨설팅 리포트 동시 생성
- **드래그 앤 드롭 에디터**: 이력서 요소를 자유롭게 이동, 크기 조절, 편집
- **3단 레이아웃**: 툴바(속성 편집) + 캔버스(이력서 편집) + 컨설팅 패널(AI 조언)
- **AI 템플릿 추천**: 프로필 분석 기반 최적 템플릿 자동 선택
- **로딩 페이지**: 채용공고 캐러셀로 대기 시간 활용
- **전문 컨설팅**: 강점/약점 분석, 프레젠테이션 전략, 표현 가이드, 1분 자기소개 스피치

**v0.8.0 최적화:**
- **API 호출 최적화**: 이력서 생성 API 호출 4회 → 2회로 감소
...
│   │   │   ├── LoadingJobsPage.jsx      (240줄) ✅ 로딩 페이지 (채용공고 캐러셀)
│   │   │   ├── ResumeEditorPage.jsx     (239줄) ✅ 드래그 앤 드롭 에디터 (3단 레이아웃)
│   │   │   └── ResumeHistoryPage.jsx    (168줄) ✅ 이력서 히스토리 목록
│   │   ├── components/
...
- **진행률 표시**:
  ...
  - 단계별 상태 메시지 (템플릿 추천 → 이력서 + 컨설팅 통합 생성)
  - 단계별 체크마크 표시

- **API 통합 (v0.8.0 최적화)**:
  - 1단계: AI 템플릿 추천 API 호출
  - 2단계: **디자인 이력서 + 컨설팅 리포트 통합 생성** (1회 API 호출)
  - 완료 후 에디터 페이지로 자동 이동
  - **이전 대비 API 호출 50% 감소** (4회 → 2회)
...
- **히스토리 목록 섹션**:
  - MongoDB에 저장된 모든 이력서 조회
  - 프로필 정보 표시 (이름, 이메일)
  - 생성 일시 표시

- **통계 정보 (v0.8.0 단순화)**:
  - 총 이력서 수 (1개 카드로 통합)
  - **이전 3개 카드에서 1개 카드로 단순화**

- **이력서 클릭 (v0.8.0 라우팅 개선)**:
  - 이력서 클릭 시 **에디터 페이지로 직접 이동**
  - React Router를 통한 자연스러운 네비게이션
  - **이전: 결과 페이지 → 개선: 에디터 페이지**

#### ✅ 라우팅 설정 (App.jsx)
- React Router DOM으로 **4개 페이지 라우팅** (v0.8.0에서 /result/:id 제거)
  - **레이아웃 없이 전체 화면 사용**:
...
- **POST /api/resume**: 이력서 저장
  - 요청: { content, profileId, type }
  - 응답: { success, message, resumeId, resume }
  - type 검증: 'basic' | 'designed' (v0.8.0에서 'custom' 제거)
  - 에러 처리 포함
...
### Step 9: API 최적화 및 코드 정리 ✅ 완료 (2025-11-30)
- [x] 이력서 생성 API 통합 (4회 호출 → 2회 호출)
- [x] Resume 모델 생성 최적화 (2개 문서 → 1개 문서)
- [x] 이력서 히스토리 라우팅 개선 (결과 페이지 → 에디터 직접 이동)
- [x] Resume 모델 필드 정리 (companyId, companyName 제거)
- [x] type enum 정리 ('custom' 제거)
- [x] ResumeResultPage.jsx 삭제
...
### Milestone 8: API 최적화 및 코드 정리 ✅ 완료 (2025-11-30)
- [x] 이력서 생성 API 통합 (generate-with-template에 컨설팅 리포트 생성 포함)
- [x] Resume 모델 생성 최적화 (2개 문서 → 1개 문서)
- [x] 이력서 히스토리 라우팅 개선 (handleResumeClick 수정)
- [x] Resume 모델 필드 정리 (companyId, companyName 제거)
- [x] type enum 정리 ('custom' 제거)
- [x] 불필요한 페이지 삭제 (ResumeResultPage.jsx)
- [x] 라우트 정리 (/result/:id 제거)
- [x] UI 단순화 (통계 카드 3개 → 1개)

**성과:**
- **API 호출 50% 감소** (4회 → 2회)
- **데이터베이스 문서 생성 50% 감소** (2개 → 1개)
- **사용자 경험 개선** (히스토리에서 에디터로 직접 이동)
